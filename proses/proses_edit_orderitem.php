...
if (!empty($_POST['edit_orderitem_validate'])) {
    ...
    if (mysqli_num_rows($select) > 0) {
        ...
        if ($query) { // Jika query berhasil
            $message = '<script>alert("Orderan berhasil diperbarui"); window.location="../?x=orderitem&order=' . $kode_order . '&meja=' . $meja . '&pelanggan=' . $pelanggan . '"</script>';
        } else { // Jika query gagal
            $message = '<script>alert("Orderan gagal diperbarui: ' . mysqli_error($conn) . '"); window.location="../?x=orderitem&order=' . $kode_order . '&meja=' . $meja . '&pelanggan=' . $pelanggan . '"</script>';
        }
    }
}
echo $message;
