...
if (!empty($_POST['ubah_password_validate'])) {
    $query = mysqli_query($conn, "SELECT * FROM tb_user WHERE username = '$_SESSION[username_tbm]' && password = '$passwordlama'");
    $hasil = mysqli_fetch_array($query);
    if ($hasil) {
        if ($passwordbaru == $repasswordbaru) {
            $query = "UPDATE tb_user SET password='$passwordbaru' WHERE username = '$_SESSION[username_tbm]'";
            echo "Query: $query<br>";
